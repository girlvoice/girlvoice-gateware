...
class BandpassIIR(wiring.Component):
    def __init__(
        ...
        if band_edges[0] < 0:
            ...
            btype = "lowpass"
        b, a = signal.butter(
            N=filter_order, btype=btype, analog=False, fs=fs, output="ba", Wn=band_edges
        )
        ...
        self.a_fp = Array(
            [
                C(int(a_i * 2**self.fraction_width), signed(sample_width))
                for a_i in a[1:]
            ]
        )
        self.b_fp = Array(
            [C(int(b_i * 2**self.fraction_width), signed(sample_width)) for b_i in b]
        )
    ...
    def elaborate(self, platform):
        ...
        y_buf = Array(
            [
                Signal(signed(self.sample_width), name=f"y_{i}")
                for i in range(len(self.a_fp))
            ]
        )
            ...
            with m.FSM() as fsm:
                ...
                with m.State("READY"):
                    ...
                    with m.If(self.source.ready):
                        m.d.sync += [
                            y_buf[i + 1].eq(y_buf[i]) for i in range(len(y_buf) - 1)
                        ]
            ...
            with m.FSM() as fsm:
                with m.State("LOAD"):
                    ...
                    with m.If(self.sink.valid):
                        ...
                        m.d.sync += [
                            y_buf[i + 1].eq(y_buf[i]) for i in range(num_taps - 1)
                        ]
                        m.d.sync += x_buf[0].eq(self.sink.payload)
                        m.d.sync += acc.eq(0)
                        m.d.sync += idx.eq(idx + 1)
