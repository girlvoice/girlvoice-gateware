...
class BandpassFIR(wiring.Component):
    def __init__(
        ...
        assert integer_width <= fraction_width, (
            f"Bitwidth {integer_width} must not exceed {fraction_width}"
        )
        ...
        self.gamma = (
            ...
            + integer_width
            + fraction_width
            + ceil(log2(sum([abs(x) for x in taps])))
        )
    ...
    def elaborate(self, platform):
        ...
        if pipeline_dsp:
                ...
                with m.State("CALC"):
                    ...
                    with m.If(idx == len(self.taps) - 1):
                        m.d.sync += self.source.payload.eq(
                            acc >> (len(acc) - self.integer_width)
                        )
            ...
            with m.If(self.sink.valid):
                m.d.sync += [
                    ring_buf[i + 1].eq(ring_buf[i]) for i in range(len(self.taps) - 1)
                ]
                m.d.sync += ring_buf[0].eq(self.sink.payload)
            ...
            y = sum(
                (ring_buf[i] * taps[i]) >> self.fraction_width
...
def run_sim():
    ...
    (t, input_samples) = generate_chirp(
        duration, fs, start_freq, end_freq, sample_width
    )
