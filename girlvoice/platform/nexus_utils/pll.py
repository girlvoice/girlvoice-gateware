...
nx_pll_param_permutation = namedtuple(
    "nx_pll_param_permutation",
    [
        "C1",
        "C2",
        "C3",
...
class NXPLL(Elaboratable):
    ...
    def register_clkin(self, clkin, freq):
        ...
        assert self.nclkins == 0, "Only one clkin allowed"
        self.clkin = Signal()
    ...
    def calc_optimal_params(self, fref, fbkdiv, M=1, BW_FACTOR=5):
        print(
            "Calculating Analog Paramters for a reference freqeuncy of "
            + str(fref * 1e-6)
            + " Mhz, feedback div "
            + str(fbkdiv)
            + ", and input div "
        ...
        for params in self.transfer_func_coefficients:
            ...
            if closed_loop_peak["peak"] < 0.8 or closed_loop_peak["peak"] > 1.35:
                continue
            ...
            for BW_CTL_BIAS in BW_CTL_BIAS_VALUES:
                for IPP_SEL in IPP_SEL_VALUES:
                    for IPI_CMP in IPI_CMP_VALUES:
                        is_valid_io_i2 = self.is_valid_io_i2(
                            IPP_CTRL, BW_CTL_BIAS, IPP_SEL, IPI_CMP
                        )
                        if is_valid_io_i2 and self.is_unique_io(is_valid_io_i2["io"]):
                            self.valid_io_i2_permutations.append(
                                io_i2(
                                    is_valid_io_i2["io"],
                                    is_valid_io_i2["i2"],
                                    IPP_CTRL,
            ...
            for CSET in CSET_VALUES:
                for CRIPPLE in CRIPPLE_VALUES:
                    for V2I_PP_RES in V2I_PP_RES_VALUES:
                        ...
                        self.transfer_func_coefficients.append(
                            nx_pll_param_permutation(
                                A * B * F + E * C,  # C1
                                A * (F * (G + 1) + B) + E * D,  # C2
                                A * (G + 1),  # C3
    ...
    def elaborate(self, platform):
        ...
        for n, (clk, f, p, m) in sorted(self.clkouts.items()):
            ...
            if platform and self.create_output_port_clocks:
                self.platform.add_platform_command(
                    "create_clock -period {} -name {} [get_pins {}.PLL_inst/CLKO{}]".format(
                        str(1 / f * 1e9),
                        self.name + "_" + n_to_l[n],
                        self.name,
                        n_to_l[n],
                    )
                )
        ...
        self.m.submodules += Instance("PLL", **self.params)
